...
- [ ] **URGENT — Settings "Keyboard Shortcuts" list is missing ⌃⌘N Toggle Notes (and ~7 more); it's a hardcoded HTML block that drifts** (logged 2026-07-07 post-S33, user-reported). The grid in `src/renderer/components/settings-panel.ts` (~line 138) is hand-maintained and was never updated for the S33 notes panel. Verified missing vs `src/main/menu.ts` accelerators: **⌃⌘N Toggle Notes**, ⌘⇧B Broadcast Input, ⌘S Save Terminal Output, ⌘N New Window, ⌘=/⌘− font size, ⌘⇧]/⌘⇧[ next/prev tab, ⌘1–9 switch-to-tab; during the fix also audit renderer-side bindings (⌘⇧K Reset Renderer?) and the configurable quick-terminal global hotkey (`main.ts:79`). Preferred fix: derive the grid from a single shared shortcuts source so it can't drift again — the command-palette entry list in `app.ts` (~line 411) already pairs labels with shortcut strings and is a natural candidate; minimal fallback: add the missing rows + a keep-in-sync comment in both files. Deliberately not fixed 2026-07-07 (user mid-live-session; avoid rebuild/relaunch churn) — **do first next session**.
