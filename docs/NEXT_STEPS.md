...
Last updated: 2026-07-07 (post-S33 — urgent shortcuts-list item added)
...
- [ ] **URGENT — Settings "Keyboard Shortcuts" list is missing ⌃⌘N Toggle Notes (and ~7 more); it's a hardcoded HTML block that drifts** (logged 2026-07-07 post-S33, user-reported). The grid in `src/renderer/components/settings-panel.ts` (~line 138) is hand-maintained and was never updated for the S33 notes panel. Verified missing vs `src/main/menu.ts` accelerators: **⌃⌘N Toggle Notes**, ⌘⇧B Broadcast Input, ⌘S Save Terminal Output, ⌘N New Window, ⌘=/⌘− font size, ⌘⇧]/⌘⇧[ next/prev tab, ⌘1–9 switch-to-tab; during the fix also audit renderer-side bindings (⌘⇧K Reset Renderer?) and the configurable quick-terminal global hotkey (`main.ts:79`). **Fix the underlying issue, not the symptom (user directive 2026-07-07): hand-adding the missing rows is NOT an acceptable end-state — the fix must guarantee future shortcuts appear automatically.** Required shape: one shortcuts registry as single source of truth (natural home: `src/shared/constants.ts`, beside IPC/DEFAULTS), holding id + label + Electron accelerator + display keys (or a converter — menu needs `CmdOrCtrl+Shift+D`, UI shows `⌘⇧D`); `menu.ts`, the `app.ts` palette entries (~line 411), and the Settings grid all read it, with the grid rendered from it at runtime. Acceptance: adding a new shortcut to the registry shows up in Settings (and palette) with zero `settings-panel.ts` edits, and no second hand-maintained list exists anywhere to forget. Deliberately not fixed 2026-07-07 (user mid-live-session; avoid rebuild/relaunch churn) — **do first next session**.
